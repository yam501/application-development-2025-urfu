...
class OrderService:
    def __init__(self, order_repository, product_repository=None, user_repository=None, email_service=None):
    ...
    async def update_order_status(self, order_id: int, new_status: str):
        print(f"DEBUG: update_order_status called with order_id={order_id}, status={new_status}")

        order = await self.order_repository.get_by_id(order_id)
        if not order:
            raise ValueError(f"Order {order_id} not found")

        print(f"DEBUG: Order found: {order.id}, user: {getattr(order, 'user', 'No user')}")
        ...
        if new_status == "shipped" and self.email_service:
            ...
            try:
                ...
                username = getattr(order.user, 'username', 'Customer') if hasattr(order, 'user') else 'Customer'
